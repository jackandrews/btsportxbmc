...
# python pipelight-xbmc.py
# "http://www.lovefilm.de/film/?token=%3Fu%3D%252Fcatalog%252Ftitle%252F326573%26m%3DGET"
...
class X11Container(QtGui.QX11EmbedContainer):
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            self, QtCore.SIGNAL('clientIsEmbedded()'), self.clientIsEmbedded)
        QtCore.QObject.connect(
            self, QtCore.SIGNAL('clientClose()'), self.clientClose)
...
class Pipelight:
    ...
    def __init__(self, pipelightName, pipelightDirectory, mozillaDirectory):
        ...
        self.enabled_systemwide = os.path.exists(
            "%s/%s" % (self.mozillaDirectory, self.pipelightName))
