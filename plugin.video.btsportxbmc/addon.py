...
class Main:
    def __init__(self):
        ...
        if param:
            ...
            self.login = BtSportLogin(
                self.settings['email'], self.settings['password'])

            home = os.getenv("HOME")
            cmd = os.path.abspath(
                "%s/.xbmc/addons/plugin.video.btsportxbmc/resources/lib/pipelight.py" % (home))

            args = [cmd, self.settings['pipelightName'], self.settings[
                'pipelightDirectory'], self.settings['mozillaDirectory'], self.settings['gpuAccel'], channel]
    ...
    def display_channels(self):
        for channel in CHANNELS:
            ...
            lnk = channel['link']
            xbmcplugin.addDirectoryItem(handle=self._handle, url="%s?channel=%s" % (
                self._path, lnk), listitem=listitem, isFolder=False)
        xbmcplugin.endOfDirectory(
            handle=self._handle, succeeded=True, cacheToDisc=False)
    ...
    def _get_settings(self):
        ...
        self.settings["pipelightName"] = __settings__.getSetting(
            "pipelightName")
        self.settings["pipelightDirectory"] = __settings__.getSetting(
            "pipelightDirectory")
        self.settings["mozillaDirectory"] = __settings__.getSetting(
            "mozillaDirectory")
